...
// Detached OCR children own their process group. If the OpenCode runtime
// exits while a review is still running, the group would otherwise be
// orphaned and keep consuming LLM tokens. Track live groups and reap them
// when the runtime shuts down. (SIGKILL/OOM cannot be intercepted; this
// covers exit, uncaught-exception exit, and SIGINT/SIGTERM.)
const liveProcessGroups = new Set<number>()
let reapHandlersInstalled = false

function signalProcessGroup(pid: number, signal: NodeJS.Signals): void {
  if (process.platform === "win32") {
    // taskkill /T kills the whole process tree; /F is the only kill it offers.
...
function reapProcessGroups(signal: NodeJS.Signals): void {
  for (const pid of liveProcessGroups) {
    try {
      signalProcessGroup(pid, signal)
    } catch {
      // The group may have exited in the meantime; nothing left to reap.
    }
  }
}

function installReapHandlers(): void {
  if (reapHandlersInstalled) return
  reapHandlersInstalled = true
  process.on("exit", () => reapProcessGroups("SIGKILL"))
  for (const signal of ["SIGINT", "SIGTERM"] as const) {
    process.on(signal, () => {
      reapProcessGroups(signal)
      // If this plugin was the only listener for the signal, restore the
      // default termination behaviour by re-raising it.
...
async function runHealthCheck(cwd: string, signal?: AbortSignal): Promise<string> {
  const [version, llm] = await Promise.allSettled([
    runOcr(["version"], { cwd, timeoutMs: 30_000, signal }),
    runOcr(["llm", "test"], { cwd, timeoutMs: 60_000, signal }),
  ])
    ...
    tool: {
      ...
      ocr_health: tool({
        ...
        async execute(_args, context) {
          const cwd = context.worktree || context.directory || worktree
          return runHealthCheck(cwd, context.abort)
        },
      }),
    },
  }
...
async function resolveSessionCwd(ctx: PluginV2.Context, sessionID: string): Promise<string> {
  try {
    ...
    // Best-effort diagnostic: the V2 context exposes no log channel, and a
    // resolution failure must not break the tool call itself.
    console.warn("open-code-review: failed to resolve session cwd; falling back to plugin directory", error)
  ...
  await ctx.tool.transform((editor) => {
    ...
    editor.add({
      ...
      execute: async (_input, toolCtx) => {
        const cwd = await resolveSessionCwd(ctx, toolCtx.sessionID)
        return { content: await runHealthCheck(cwd) }
      },
    })
  })
